...
function ParticipantPlayQuiz() {
  ...
  const [startTime, setStartTime] = useState(null); // New state to store start time
  const [endTime, setEndTime] = useState(null); // New state to store end time
  ...
  const handleNextQuestion = async () => {
    ...
    if (correctAnswers + wrongAnswers === questions.length - 1) {
      setEndTime(new Date()); // Set end time when the last question is answered
  ...
  const updateParticipantMarks = async () => {
    try {
      ...
      if (!participantSnapshot.empty) {
        ...
        const updatedMarks = totalMarks + 2;
        ...
        <Row className="justify-content-center" style={{ marginTop: '30px' }}>
          <Col>
            ...
            <div className='text-center mt-3'>
              {!showResult && !completedQuiz && (
                <Button variant="primary" onClick={handleNextQuestion} disabled={loading}>
                  {correctAnswers + wrongAnswers === questions.length - 1 ? 'Submit' : 'Next'}
                </Button>
              )}
            </div>
          </Col>
